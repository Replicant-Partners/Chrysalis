...
# This document is retained for historical reference only. For active modes see `ExistingModes/` and the Kilocode sync paths (CLI: `~/.kilocode/modes.yaml`, VS Code: `~/.config/Code/User/globalStorage/kilocode.kilo-code/settings/custom_modes.yaml`).
...
customModes:
  ...
  - slug: architect
    ...
    groups:
      ...
      **CRITICAL: Never provide level of effort time estimates (e.g., hours,
      days, weeks) for tasks. Focus solely on breaking down the work into clear,
      actionable steps without estimating how long they will take.**


      Unless told otherwise, if you want to save a plan file, put it in the
      /plans directory
    source: project
