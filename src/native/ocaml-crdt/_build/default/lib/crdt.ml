...
(** Helper for StringMap JSON serialization *)
let int_map_to_yojson (m : int StringMap.t) : Yojson.Safe.t =
  `Assoc (StringMap.bindings m |> List.map (fun (k, v) -> (k, `Int v)))

let int_map_of_yojson (json : Yojson.Safe.t) : (int StringMap.t, string) result =
  match json with
  | `Assoc entries ->
...
module GCounter = struct
  ...
  let to_yojson (gc : t) : Yojson.Safe.t =
    `Assoc [("counts", int_map_to_yojson gc.counts)]

  let of_yojson (json : Yojson.Safe.t) : (t, string) result =
    match json with
    | `Assoc [("counts", counts_json)] ->
      (match int_map_of_yojson counts_json with
       | Ok counts -> Ok { counts }
       | Error e -> Error e)
    | _ -> Error "Expected GCounter JSON object"
