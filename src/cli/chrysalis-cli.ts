...
function loadConverterV2OrExit(): any {
  try {
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    return require('../converter/ConverterV2').ConverterV2;
  } catch {
    console.error('\n❌ ConverterV2 is not available in this build.');
    console.error('   This usually means the v2 sync/merge subsystem was removed or is mid-migration.');
    console.error('   You can still use v1 morphing (framework conversion) without ConverterV2.\n');
    process.exit(1);
  }
}
  ...
  .action(async (options) => {
    try {
      ...
      if (useV2 && options.type) {
        ...
        const ConverterV2 = loadConverterV2OrExit();
...
program
  .command('gateway')
  .description('Interact with the Go LLM gateway')
  .addHelpText('after', `

Examples:
  $ chrysalis gateway health
  $ chrysalis gateway agents
  $ chrysalis gateway chat --agent-id ada --prompt "hello"\
  ...
  .action(async (options) => {
    try {
      const client = new GatewayLLMClient({
        baseUrl: options.url || process.env.GATEWAY_BASE_URL || 'http://localhost:8080',
        authToken: options.token || process.env.GATEWAY_AUTH_TOKEN,
      ...
      console.error('\n❌ Gateway health check failed:', error.message);
      process.exit(1);
    }
  });

program
  .command('gateway agents')
  .description('List agents known to the gateway')
  .option('--url <url>', 'Gateway base URL')
  .option('--token <token>', 'Gateway auth token')
  .action(async (options) => {
    try {
...
program
  .command('gateway chat')
  .description('Send a single chat prompt to the gateway')
  .requiredOption('--agent-id <id>', 'Agent ID registered in the gateway')
  .option('--prompt <text>', 'Prompt text')
  .option('--prompt-file <path>', 'Read prompt from a file')
  .option('--url <url>', 'Gateway base URL')
  .option('--token <token>', 'Gateway auth token')
  .option('--model <model>', 'Model override')
  .option('--temperature <n>', 'Temperature', (v) => Number(v))
  .option('--max-tokens <n>', 'Max tokens', (v) => Number(v))
  .action(async (options) => {
    try {
      const prompt = options.prompt
        ? String(options.prompt)
        : options.promptFile
          ? await fs.readFile(String(options.promptFile), 'utf-8')
          : '';

      if (!prompt.trim()) {
        console.error('\n❌ Missing prompt. Provide --prompt or --prompt-file.\n');
        process.exit(1);
      }
      ...
      const client = new GatewayLLMClient({
        baseUrl: options.url || process.env.GATEWAY_BASE_URL || 'http://localhost:8080',
        authToken: options.token || process.env.GATEWAY_AUTH_TOKEN,
        defaultModel: options.model || process.env.LLM_DEFAULT_MODEL,
      });
      ...
      const response = await client.chat({
        ...
        model: options.model,
        temperature: typeof options.temperature === 'number' && !Number.isNaN(options.temperature)
          ? options.temperature
          : undefined,
        maxTokens: typeof options.maxTokens === 'number' && !Number.isNaN(options.maxTokens)
          ? options.maxTokens
          : undefined,
      });
      ...
      console.error('\n❌ Gateway chat failed:', error.message);
      process.exit(1);
    }
  });
