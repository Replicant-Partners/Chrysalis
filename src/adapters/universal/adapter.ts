...
/**
 * LLM provider interface for adapter
 */
 ...
 */
export interface MorphingResult extends TranslationResult {
...
export class UniversalAdapter {
  ...
  private config: Required<UniversalAdapterConfig>;
  ...
  async validate(
    ...
    const prompt = buildValidationPrompt(agent, protocolEntry, spec.content);
  ...
  async discoverCapabilities(protocol: string): Promise<ProtocolCapabilities> {
    ...
    const prompt = buildCapabilityDiscoveryPrompt(protocolEntry, spec.content);
  ...
  listProtocols(): Array<{
    ...
    return getRegisteredProtocols().map(id => {
      const entry = PROTOCOL_REGISTRY[id];
      return {
        id,
        name: entry.name,
   ...
   */
  private getProtocolEntry(protocol: string): ProtocolEntry {
   ...
   */
  private createMinimalSpec(entry: ProtocolEntry): string {
...
export function createUniversalAdapter(
  llm: LLMProvider,
  options?: Omit<UniversalAdapterConfig, 'llm'>
): UniversalAdapter {
