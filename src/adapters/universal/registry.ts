 ...
 */
export function getProtocolsByTrustLevel(level: ProtocolEntry['trustLevel']): string[] {
  return Object.entries(PROTOCOL_REGISTRY)
...
export function isProtocolRegistered(id: string): boolean {
  return id in PROTOCOL_REGISTRY;
}
...
export function getSemanticHints(protocol: string): SemanticHints | undefined {
  return PROTOCOL_REGISTRY[protocol]?.semanticHints;
}
