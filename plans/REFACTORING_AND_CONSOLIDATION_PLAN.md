...
2.  **Test Passing**: The regression test suite (`tests/integration/adapters.test.ts`) passes using the new `UniversalAdapterV2`.
3.  **Registry Adoption**: All protocol definitions are sourced exclusively from `src/adapters/universal/registry-v2.ts`.
4.  **OpenHands Bridge**: A defined interface exists for the TypeScript adapter to invoke the Python `FlowExecutor`.

## 2. Technical Approach & Architecture

### Pattern Adoption
The `UnifiedAdapter` interface (in `src/adapters/unified-adapter.ts`) will serve as the facade. However, instead of wrapping legacy classes, it will now exclusively wrap `UniversalAdapterV2`.
*   **Translation**: All `toUniversalMessage` and `fromUniversalMessage` calls will delegate to `UniversalAdapterV2.translate()`, which uses the LLM and semantic prompts.
*   **Validation**: `validate()` will use `UniversalAdapterV2.validate()`.
...
*   **Routing**: The `UniversalAdapterV2` will detect if a task requires complex reasoning (loops, multi-step planning).
...
*   **[High] Fix Index Exports**: Ensure `src/adapters/index.ts` cleanly exports `UniversalAdapterV2` and related types, with no lingering references to deleted files. (Completed)
...
*   **[Medium] Verify Unified Wrapper**: Ensure `createUnifiedAdapter` in `unified-adapter.ts` correctly instantiates `UniversalAdapterV2` when no legacy adapter is provided.
...
*   **[High] Registry Enforcement**: Refactor `UniversalAdapterV2` to throw an error if a protocol is not in `PROTOCOL_REGISTRY_V2`.
...
*   **[Medium] Flow Execution Hook**: Add a method `executeFlow(graph: FlowGraph)` to `UniversalAdapterV2` that calls the Python bridge.
...
| **Loss of Nuance** | Generic LLM translation misses protocol-specific edge cases (e.g., MCP resource subscription). | Medium | Enhance `semanticHints` in `registry-v2.ts` with explicit mapping rules for edge cases. |
